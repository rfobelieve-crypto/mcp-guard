...
> 證據：`async_adbutils、posthog>=6.7.6、pydantic>=2.11.10、rich>=14.1.0、InquirerPy>=0.3.4、arize-phoenix>=14.4.0…`

### 🔵 低｜[權限] 會讀取環境變數（符合宣稱用途）

環境變數常存放 API 金鑰。確認它只讀自己需要的那幾個。「開發框架／工具鏈」類工具本來就需要這個能力，屬預期範圍；重點是你**知情**並給予對應的信任。

> 證據：`mobilerun/agent/droid/droid_agent.py、mobilerun/agent/fast_agent/fast_agent.py、mobilerun/agent/manager/manager_agent.py、mobilerun/agent/utils/llm_picker.py、mobilerun/agent/utils/oauth/anthropic_oauth_llm.py`
...
> 證據：`SKILL.md（Agent Skill 指令（SKILL.md））`
...
> 證據：`registry 名稱 io.github.droidrun/mobilerun`

### ⚪ 資訊｜[身分] 倉庫基本資料

⭐ 9359｜fork 1000｜語言 Python｜建立 2025-04-12｜最後推送 2026-09-11
