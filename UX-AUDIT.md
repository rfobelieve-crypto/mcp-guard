...
> ## ⚠️ 更正（2026-08-01）：下表的 `/registry/` 分數是**錯的**
>
> 原始量測用的本機伺服器（`python3 -m http.server`）**不做壓縮**，
> 而 Vercel 對文字回應一律 gzip/brotli。用會壓縮的伺服器重量之後：
>
> | 頁面 | 原本記錄(無壓縮) | **實際(有壓縮，等同正式環境)** |
> |---|---|---|
> | `/` | 97 | **100** |
> | `/registry/` | 54 | **98** |
> | `/pick/` | 100 | **100** |
...
> | `/trust/` | — | **100** |
>
> `/registry/` 壓縮後的實際傳輸量是 **約 105 KB**，不是 1.04 MB。
>
> **因此「`/registry/` 是全案效能瓶頸」這個結論不成立，全站沒有效能問題。**
> 這個錯誤影響了後續判斷（`PRODUCT-ANALYSIS.md` 也引用過），
> 教訓是：**量測環境與正式環境的差異本身就是一個變因**，
> 拿本機不壓縮的數字去推論正式環境，會憑空製造出一個不存在的瓶頸。
>
> 唯一仍然真實的成本是**版面計算**：178 列的完整發現內容即使收合也在 DOM 裡，
> 實測 Style & Layout 佔 1,149ms。已用 `content-visibility:auto` 處理，
> 降到 366ms、TBT 280→130ms、主執行緒 2.1s→0.9s。

以下為原始（無壓縮）量測記錄，保留供對照：
