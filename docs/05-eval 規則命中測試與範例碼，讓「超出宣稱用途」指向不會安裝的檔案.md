...
（**追記 2026-09-16**：重掃 `d154a0a` 時總數已成長為 30 筆——`BetterDB-inc/monitor` 與
`HeyPuter/puter` 各新增一筆，兩筆的證據都落在真正會出貨的原始碼
（`apps/api/src/auth/better-auth-esm.ts`、`src/backend/core/http/middleware/puterSite.ts`），
**不屬於本文討論的非出貨路徑那一類**。下表「全部落在測試／基準／範例」的那 6 筆維持不變，
本文的論點與名單都不受影響，僅分母由 28 變成 30。）
